...
def generate_linoss_inputs(B, T, H, P, dtype, with_initial_state=False):
    ...
    if with_initial_state:
        h0 = torch.randn((B, 4, P), dtype=dtype, device=device) * 0.1
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(*test, id="B{}-T{}-H{}-P{}-{}-{}".format(*test))
        for test in [
            (1, 64, 32, 16, 'IM', torch.float),
            (2, 128, 64, 32, 'IM', torch.float),
            (2, 100, 64, 32, 'IM', torch.float),
            (1, 64, 32, 16, 'IMEX', torch.float),
            (2, 128, 64, 32, 'IMEX', torch.float),
            (2, 100, 64, 32, 'IMEX', torch.float),
        ]
    ],
)
def test_chunk_fwd(
    ...
    """Test chunk forward pass against naive."""
    os.environ['TRITON_F32_DEFAULT'] = 'ieee'
    ...
    assert_close(f'  chunk fwd ({discretization})', ref, tri, 0.01)
...
def test_chunk_bwd(
    ...
    """Test chunk backward pass: gradients from naive vs chunk."""
    os.environ['TRITON_F32_DEFAULT'] = 'ieee'
...
def test_chunk_with_initial_state(
    B: int,
    T: int,
    H: int,
    P: int,
    ...
    """Test chunk forward with initial state and final state."""
    os.environ['TRITON_F32_DEFAULT'] = 'ieee'
    ...
    assert_close(f'  chunk fwd+h0 ({discretization})', ref, tri, 0.01)
    if ref_ht is not None and tri_ht is not None:
        assert_close(f'  chunk ht ({discretization})', ref_ht, tri_ht, 0.01)
