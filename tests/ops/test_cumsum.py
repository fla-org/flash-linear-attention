...
def test_cumsum_local_vector(B, T, H, D, dtype, reverse, chunk_size):

    s = torch.randn((B, T, H, D), dtype=dtype, device=device).requires_grad_()
    ref = cumsum_local_reference(s, reverse=reverse, head_first=False, chunk_size=chunk_size)
    tri = chunk_local_cumsum(s, reverse=reverse, head_first=False, chunk_size=chunk_size)
...
def test_cumsum_local_scalar(B, T, H, dtype, reverse, chunk_size):
...
def test_cumsum_global_vector(B, T, H, D, dtype, reverse):
    s = torch.randn((B, T, H, D), dtype=dtype, device=device).requires_grad_()
    ref = cumsum_global_reference(s, reverse=reverse, head_first=False)
    tri = chunk_global_cumsum(s, reverse=reverse, head_first=False)
...
def test_cumsum_global_scalar(B, T, H, dtype, reverse):
