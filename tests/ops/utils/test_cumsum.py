...
@pytest.mark.parametrize(
    ('B', 'T', 'H', 'C', 'D', 'dtype'),
    [
        pytest.param(*test, id="B{}-T{}-H{}-C{}-D{}-{}-hf".format(*test))
        for test in [
            (1, 63, 2, 16, 30, torch.float),
            (2, 500, 4, 32, 60, torch.float),
            (2, 1000, 5, 64, 128, torch.float),
            (3, 1024, 6, 64, 500, torch.float),
            (4, 2048, 8, 128, 1024, torch.float),
...
def test_local_cumsum_head_first(
    B: int,
    T: int,
    H: int,
    C: int,
    ...
    torch.manual_seed(42)
    s = torch.randn(B, H, T, dtype=dtype).to(device)
    ref = torch.cat([s[:, :, i:i+C].float().cumsum(2) for i in range(0, T, C)], 2)
    tri = chunk_local_cumsum(s, chunk_size=C, head_first=True)
    assert_close('local_cumsum_head_first', ref, tri, 1e-3)

    s = torch.randn(B, H, T, D, dtype=dtype).to(device)
    ref = torch.cat([s[:, :, i:i+C].float().cumsum(2) for i in range(0, T, C)], 2)
    tri = chunk_local_cumsum(s, chunk_size=C, head_first=True)
    assert_close('local_cumsum_head_first', ref, tri, 1e-3)
...
@pytest.mark.parametrize(
    ('B', 'T', 'H', 'C', 'D', 'dtype'),
    [
        pytest.param(*test, id="B{}-T{}-H{}-C{}-D{}-{}-hf-rev".format(*test))
        for test in [
            (1, 63, 2, 16, 30, torch.float),
            (2, 500, 4, 32, 60, torch.float),
            (2, 1000, 5, 64, 128, torch.float),
...
def test_local_reversed_cumsum_head_first(
    B: int,
    T: int,
    H: int,
    C: int,
    ...
    torch.manual_seed(42)
    s = torch.randn(B, H, T, dtype=dtype).to(device)
    ref = torch.cat([reversed_cumsum(s[:, :, i:i+C], 2) for i in range(0, T, C)], 2)
    tri = chunk_local_cumsum(s, chunk_size=C, reverse=True, head_first=True)
    assert_close('local_reversed_cumsum_head_first', ref, tri, 1e-3)
