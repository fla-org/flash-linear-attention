...
def test_deltaformer_attn(
    ...
    if B > 1:
        q_packed = torch.cat([q[i] for i in range(B)], dim=0).unsqueeze(0).detach().clone().requires_grad_(True)
        k_packed = torch.cat([k[i] for i in range(B)], dim=0).unsqueeze(0).detach().clone().requires_grad_(True)
        v_packed = torch.cat([v[i] for i in range(B)], dim=0).unsqueeze(0).detach().clone().requires_grad_(True)
        beta_packed = torch.cat([beta[i] for i in range(B)], dim=0).unsqueeze(0).detach().clone().requires_grad_(True)

        cu_seqlens = torch.arange(0, B*T + 1, T, dtype=torch.int32, device=device)

        grad_output_packed = torch.cat([do[i] for i in range(B)], dim=0).unsqueeze(0)
        ...
        assert_close("varlen_o", ref, tri_varlen_reshaped, 0.006)
