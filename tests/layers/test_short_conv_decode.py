...
@torch.no_grad()
def test_short_conv_packed_decode(layer_cls, metadata):
    ...
    cu_seqlens_cpu = torch.arange(4, dtype=torch.int32)
    ...
    sync_mode = torch.cuda.get_sync_debug_mode()
    try:
        if metadata != 'infer':
    ...
    assert scalar_reads == (0 if metadata == 'hint' else 1)
