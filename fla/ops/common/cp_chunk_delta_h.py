...
def get_cp_cu_seqlens(cu_seqlens, world_size=None, rank=None, group=None, kernel_size=None):
    ...
    is_last_rank_list = []
    pre_num_ranks_list = []
    pre_num_conv_tokens_list = []
    is_first_rank_list = []
    post_num_ranks_list = []
    for idx in range(len(cu_seqlens_list) - 1):
        left = cu_seqlens_list[idx]
        right = cu_seqlens_list[idx + 1]
        if left >= end:
            break
    ...
    if kernel_size is not None:
        num_cat = min(pre_num_conv_tokens, kernel_size - 1)
        if num_cat > 0:
            cu_seqlens_conv1d = cu_seqlens.clone()
            cu_seqlens_conv1d[1:] += num_cat
        else:
...
def pre_process_fwd_bwd_kernel_stage2(
    ...
    for _i_t in range(NT):
        ...
        if USE_G:
            ...
            b_gk_last = tl.load(gk + (bos + last_idx) * H*K + i_h * K + row, mask=(row < K), other=0.)
            if USE_EXP2:
...
def chunk_gated_delta_rule_bwd_dhu_pre_process(
    ...
    ag_dhm, _ = all_gather(dhm, group=context.group)
    if context.post_num_ranks > 0:
        def grid(meta): return (triton.cdiv(V, meta['BV']), H)
        merge_fwd_bwd_kernel[grid](
            dht[-1],
            ag_dhm,
