...
def _load_state_vk(
    ...
    A transposed block_ptr (last-dim stride 1 along V, row stride VS) would let
    CANN leftover DMA alias the next row; keep last-dim contiguous and boundary_check on.
    """
...
def chunk_fwd_kernel_o_npu(
    ...
    for task_id in tl.range(core_id, task_num, num_core):
        ...
        # last-dim of h is logical K/V; leftover DMA is clipped by boundary_check.
        ...
        for i_k in range(tl.cdiv(K, BK)):
            ...
            # zero leftover lanes of the register tile (varlen / T,K,V tail).
            if MASK_LEFTOVER:
                o_k = i_k * BK + tl.arange(0, BK)
...
def chunk_fwd_o_npu(
    ...
    # aligned T skips leftover-mask copies and keeps BK/BV=128 Cube tiles.
...
def chunk_bwd_kernel_dqkwg_full_npu(
    ...
    for task_id in tl.range(core_id, task_num, num_core):
        ...
        for i_k in range(tl.cdiv(K, BK)):
            ...
            # Masked stores skip leftover T/K lanes (partial last tile / varlen).
            o_k = i_k * BK + tl.arange(0, BK)
            m_qk = m_t[:, None] & (o_k < K)[None, :]
            tl.store(
...
def chunk_bwd_dv_local_npu(
    ...
    if mask_leftover and not use_full:
        BK = min(64, BK)
        BV = min(64, BV)
    dv = do.new_zeros(B, T, HV, V) if mask_leftover else do.new_empty(B, T, HV, V)
    kernel_kwargs = {
        'q': q,
        'k': k,
        'g': g_arg,
...
def chunk_bwd_dqkwg_npu(
    ...
    if mask_leftover:
        ...
        dw = w.new_zeros(B, T, HV, K) if use_dw else None
    else:
        dq = q.new_empty(B, T, HV, K)
        dk = k.new_empty(B, T, HV, K)
        dw = w.new_empty(B, T, HV, K) if use_dw else None
    ...
    if dg is not None:
        ...
        if use_full:
            ...
            chunk_bwd_kernel_dg_hdh_npu[(num_vec,)](
                ...
                IS_VARLEN=cu_seqlens is not None,
                MASK_LEFTOVER=npu_leftover_mask(
                    T=T, BT=BT, varlen=cu_seqlens is not None,
                ),
            )
    return dq, dk, dw, dg
