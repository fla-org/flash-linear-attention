...
def get_cp_cu_seqlens(
    ...
    # 4. Compute local cu_seqlens on CPU (int64/long)
    ...
    # Transfer to GPU (int64/long, small tensor, fast transfer)
    # non_blocking=True can further hide latency in CUDA streams
    local_cu_seqlens_gpu = local_cu_seqlens_cpu.to(
        device=cu_seqlens.device, dtype=torch.long, non_blocking=True
    )
