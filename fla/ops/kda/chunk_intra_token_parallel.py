...
def chunk_kda_fwd_kernel_intra_token_parallel(
    ...
    if IS_VARLEN:
        i_n = 0
        left, right = 0, N
        for _ in range(tl.ceil(tl.log2(N.to(tl.float32))).to(tl.int32)):
            if left < right:
                mid = (left + right) // 2
    ...
    p_q = tl.make_block_ptr(q + i_t * H*K, (H, K), (K, 1), (i_hg * BH, 0), (BH, BK), (0, 1))
    p_k = tl.make_block_ptr(k + i_t * H*K, (H, K), (K, 1), (i_hg * BH, 0), (BH, BK), (0, 1))
    p_g = tl.make_block_ptr(g + i_t * H*K, (H, K), (K, 1), (i_hg * BH, 0), (BH, BK), (0, 1))
    ...
    for j in range(i_ts, min(i_ts + BC, T)):
        p_kj = tl.make_block_ptr(k + j * H*K, (H, K), (K, 1), (i_hg * BH, 0), (BH, BK), (0, 1))
        p_gj = tl.make_block_ptr(g + j * H*K, (H, K), (K, 1), (i_hg * BH, 0), (BH, BK), (0, 1))
        # [BH, BK]
