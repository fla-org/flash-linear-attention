...
from fla.utils import IS_AMD, IS_NVIDIA_HOPPER, autotune_cache_kwargs, check_shared_mem

BK_LIST = ([32, 64, 128] if check_shared_mem() else [32, 64]) if IS_AMD else ([32, 64] if check_shared_mem() else [16, 32])
BV_LIST = [64, 128] if check_shared_mem('ampere') else [16, 32]
NUM_WARPS = [2, 4, 8, 16] if IS_AMD else ([2, 4] if IS_NVIDIA_HOPPER else [2, 4, 8])
...
@fla_cache_autotune(
    configs=[
        ...
        for num_stages in ([1, 2, 3] if IS_AMD else [2, 3, 4])
    ],
