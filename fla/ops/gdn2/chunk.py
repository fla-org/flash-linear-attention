    ...
    **kwargs,
    ...
    Args:
        ...
        state_v_first: store the recurrent state in ``[V, K]`` layout instead
            of the default ``[K, V]``.
        cp_context: context-parallel context for distributed training across
            multiple devices. When provided, ``initial_state`` and
            ``output_final_state`` are not supported, and ``cu_seqlens`` /
            ``cu_seqlens_cpu`` are overridden by the context. Default: ``None``.
