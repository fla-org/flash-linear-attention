...
def chunk_oja2_fwd_kernel_h_blockdim64(
    v,
    u,
    w,
    k_new,
...
def chunk_oja2_fwd_h(
    ...
    chunk_oja2_fwd_kernel_h_blockdim64[grid](
        v=v,
        u=u,
        w=w,
        k_new=k_new,
...
@triton.jit(do_not_specialize=['T'])
def chunk_oja2_bwd_kernel_dhu_blockdim64(
    q,
    vg,
    w,
    gv,
...
def chunk_oja2_bwd_dhu(
    ...
    def grid(meta): return (triton.cdiv(K, meta['BK']), N*H)
    chunk_oja2_bwd_kernel_dhu_blockdim64[grid](
        q=q,
        vg=vg,
        w=w,
        gv=gv,
...
@triton.jit(do_not_specialize=['T'])
def chunk_oja2_bwd_kernel_dvwg_h(
    k,
    v,
    gv,
    h,
...
def chunk_oja2_bwd_dvwg_h(
    ...
    grid = (NV, NT, B * H)
    chunk_oja2_bwd_kernel_dvwg_h[grid](
        k=k,
        v=v,
        gv=gv,
        h=h,
