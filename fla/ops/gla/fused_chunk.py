    ...
    def forward(ctx, q, k, v, g, scale, initial_state, output_final_state):
        ...
        if version.parse(triton.__version__) < version.parse('2.2.0'):
            ...
            warnings.warn(
                ...
                "For optimal performance, it is recommended to install Triton>=2.2.0 (if possible).", stacklevel=2,
            )
            CHECK = True
...
def fused_chunk_gla(
    ...
    if head_first:
        warnings.warn(
            ...
            "Please use head_first=False for now instead.", stacklevel=2,
        )
        q, k, v, g = (rearrange(x, 'b h t ... -> b t h ...') for x in (q, k, v, g))
    ...
    if scale == -1:
        scale = q.shape[-1] ** -0.5
    seq_len = q.shape[-2]
    q, k, v, g = (pad(x) for x in [q, k, v, g])
