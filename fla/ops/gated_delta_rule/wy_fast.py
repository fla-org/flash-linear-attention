...
@triton.autotune(
    ...
    key=['H', 'K', 'BT', 'USE_OFFSETS', 'HEAD_FIRST'],
)
...
def chunk_scaled_dot_kkt_fwd_kernel(
    ...
    b_Aw = tl.zeros([BT, BT], dtype=tl.float32)
    if HEAD_FIRST:
    ...
    b_Aw = tl.where(o_t[:, None] > o_t[None, :], b_Aw, 0)
...
@triton.autotune(
    ...
    key=['H', 'K', 'V', 'BT', 'BK', 'BV', 'USE_OFFSETS', 'HEAD_FIRST'],
)
@triton.jit(do_not_specialize=['T'])
...
def chunk_scaled_dot_kkt_fwd(
    ...
    Aw = torch.empty(B, *((H, T) if head_first else (T, H)), BT, device=k.device, dtype=output_dtype)
    Au = torch.empty(B, *((H, T) if head_first else (T, H)), BT, device=k.device, dtype=output_dtype)
...
def fwd_prepare_wy_repr(
    ...
    T = k.shape[2] if head_first else k.shape[1]
    BT = min(chunk_size, max(triton.next_power_of_2(T), 16))
