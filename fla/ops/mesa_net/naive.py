...
def naive_mesa_net_CG(q, k, v, g, lamb, beta, chunk_size, max_CG_iteration=30):
    ...
    q_chunk, k_chunk, v_chunk, g_chunk, lamb_chunk, beta_chunk = map(chunk_fn, [q, k, v, g, lamb, beta])
    ...
    x = q_chunk / (diag_H.unsqueeze(-2) + lamb_chunk[..., None])
    r = q_chunk - (x * chunk_decay_q[..., None]) @ h_kk_all - ((x @ k_chunk.transpose(-2, -1))
                                                               * pairwise_decay) @ k_chunk - (lamb_chunk[..., None] * x)
    p = r.clone()
    delta_old = (r * r).sum(-1)
    ...
    for i in range(max_CG_iteration):
        q = (p * chunk_decay_q[..., None]) @ h_kk_all + ((p @ k_chunk.transpose(-1, -2))
                                                         * pairwise_decay) @ k_chunk + (lamb_chunk[..., None] * p)
...
def naive_mesa_net_exact(q, k, v, g, lamb, beta, h_kk_init=None, h_kv_init=None):
    ...
    for i in range(L):
        ...
        h_kk_all[:, i] = h_kk
        h_kv_all[:, i] = h_kv
    q_star_gold = torch.linalg.solve(h_kk_all + lamb[..., None, None] *
                                     torch.eye(d, device=q.device)[None, None, None, :, :], q)
