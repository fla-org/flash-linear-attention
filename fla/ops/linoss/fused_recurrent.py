...
def fused_recurrent_linoss_fwd_kernel(
    ...
    o,          # output: (B, T, H)
    h0,         # initial state: (N, 4, P) or None
    ht,         # final state: (N, 4, P) or None
    T,
    H: tl.constexpr,
    P: tl.constexpr,
    BP: tl.constexpr,
    ...
    if USE_INITIAL_STATE:
        b_h1_re += tl.load(h0 + i_b * 4 * P + o_p, mask=mask_p, other=0.).to(tl.float32)
        b_h1_im += tl.load(h0 + i_b * 4 * P + P + o_p, mask=mask_p, other=0.).to(tl.float32)
        b_h2_re += tl.load(h0 + i_b * 4 * P + 2 * P + o_p, mask=mask_p, other=0.).to(tl.float32)
        b_h2_im += tl.load(h0 + i_b * 4 * P + 3 * P + o_p, mask=mask_p, other=0.).to(tl.float32)
    ...
    if STORE_FINAL_STATE:
        tl.store(ht + i_b * 4 * P + o_p, b_h1_re.to(ht.dtype.element_ty), mask=mask_p)
        tl.store(ht + i_b * 4 * P + P + o_p, b_h1_im.to(ht.dtype.element_ty), mask=mask_p)
        tl.store(ht + i_b * 4 * P + 2 * P + o_p, b_h2_re.to(ht.dtype.element_ty), mask=mask_p)
        tl.store(ht + i_b * 4 * P + 3 * P + o_p, b_h2_im.to(ht.dtype.element_ty), mask=mask_p)
...
def fused_recurrent_linoss_fwd(
    ...
    o = x.new_zeros(B, T, H)
    final_state = x.new_empty(B, 4, P) if output_final_state else None
    ...
    def backward(ctx, do, dht=None):
        ...
        with torch.enable_grad():
            ...
            o, ht = _linoss_recurrent_torch(
                ...
                initial_state, discretization, output_final_state,
            )

            targets = [o] if ht is None else [o, ht]
            grads = [do] if ht is None else [do, dht]
...
def _linoss_recurrent_torch(
    ...
    h1_re = x_f.new_zeros(Bat, P)
    h1_im = x_f.new_zeros(Bat, P)
    h2_re = x_f.new_zeros(Bat, P)
    h2_im = x_f.new_zeros(Bat, P)

    if initial_state is not None:
        h1_re = initial_state[:, 0].float()
        h1_im = initial_state[:, 1].float()
        h2_re = initial_state[:, 2].float()
        h2_im = initial_state[:, 3].float()
    ...
    for t in range(T):
        ...
        y_t = (torch.einsum('hp,bp->bh', C_re_f, h2_re)
               - torch.einsum('hp,bp->bh', C_im_f, h2_im)
               + d_skip.float().unsqueeze(0) * x_f[:, t])
        outputs.append(y_t)
    ...
    if output_final_state:
        final_state = torch.stack([h1_re, h1_im, h2_re, h2_im], dim=1)
...
def fused_recurrent_linoss(
    ...
    """
    if any(t.requires_grad for t in (x, B_re, B_im, C_re, C_im, a_diag, dt, d_skip)):
