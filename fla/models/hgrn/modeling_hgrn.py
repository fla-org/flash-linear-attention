...
class HGRNVisionPreTrainedModel(PreTrainedModel):
    config_class = HGRNVisionConfig
...
class HGRNVisionEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList([
            HGRNVisionBlock(config, layer_idx) 
...
class HGRNVisionModel(HGRNVisionPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False):
        super().__init__(config)
        self.config = config
        self.embeddings = ImageEmbeddings(config, use_mask_token=use_mask_token)
        self.encoder = HGRNVisionEncoder(config)
...
class HGRNForImageClassification(HGRNVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_classes
        self.backbone = HGRNVisionModel(config, add_pooling_layer=True) # Here we should use mean pooling
...
class HGRNForMaskedImageModeling(HGRNVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.backbone = HGRNVisionModel(config, add_pooling_layer=False, use_mask_token=True) 
