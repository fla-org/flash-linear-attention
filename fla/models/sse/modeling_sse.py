...
class SSEBlock(GradientCheckpointingLayer):
    ...
    def __init__(self, config: SSEConfig, layer_idx: int):
        ...
        self.attn_norm = (RMSNorm if config.fuse_norm else nn.RMSNorm)(config.hidden_size, eps=config.norm_eps, dtype=torch.float32)
        ...
        self.mlp_norm = (RMSNorm if config.fuse_norm else nn.RMSNorm)(config.hidden_size, eps=config.norm_eps, dtype=torch.float32)
        self.mlp = SSEMLP(
            hidden_size=config.hidden_size,
...
class SSEModel(SSEPreTrainedModel):
    ...
    def __init__(self, config: SSEConfig):
        ...
        self.norm = (RMSNorm if config.fuse_norm else nn.RMSNorm)(config.hidden_size, eps=config.norm_eps, dtype=torch.float32)
