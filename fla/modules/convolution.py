...
def causal_conv1d_bwd_kernel(
    ...
    if USE_FINAL_STATE and i_t * BT + BT >= T-W:
        ...
        **kwargs,
        ...
        if 'use_fast_conv1d' in kwargs:
            warnings.warn(
                ...
                "Please use the `backend` parameter instead.", stacklevel=2,
            )
        import os
        ...
        if backend == 'cuda' and causal_conv1d_fn is None:
            warnings.warn(
                ...
                "Consider installing `causal_conv1d` to enable the CUDA backend.", stacklevel=2,
            )
            self.backend = 'triton'
