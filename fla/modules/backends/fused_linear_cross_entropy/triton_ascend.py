...
def _vocab_block_size(vocab_size: int, num_rows: int, is_backward: bool = True) -> int:
...
def logsumexp_fwd(
    ...
    x = x.view(-1, shape[-1])
    N, D = x.shape
    B = _vocab_block_size(vocab_size=D, num_rows=N, is_backward=False)
...
def fused_linear_cross_entropy_forward(
    ...
    BV = _vocab_block_size(vocab_size=V, num_rows=N)
