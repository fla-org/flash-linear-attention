...
def _chunk_size(T: int, BT: int) -> int:
    BT = min(max(BT, 1), 64)
...
def _max_axis_chunks(grid_dim0: int, batch: int = 1) -> int:
...
def _launch_bwd_dx_core(
    ...
    if BD is None:
        if dht is not None:
            ...
            BD, BT = _tile_config(T, BT, D, dy.dtype, None)
    ...
    max_nt = _max_axis_chunks(triton.cdiv(D, BD), B)
    kernel_kwargs = dict(
        dy=dy,
        weight=weight,
        dht=dht,
...
def _launch_fwd_core_scalar(
    ...
    # fallback, and the bf16 big tiles from _tile_config overflow Ascend UB under the
    ...
    BD, BT = _tile_config(T, BT, D, torch.float32, initial_state)
...
def _launch_fwd_core(
    ...
    max_nt_grid = _max_axis_chunks(grid_dim0, B)
...
def compute_dh0(
    ...
    max_n = _max_axis_chunks(triton.cdiv(D, BD))
    kernel_kwargs = dict(
        dy=dy,
...
def causal_conv1d_update_states(
    ...
    BW = triton.next_power_of_2(W)
    grid_dim0 = triton.cdiv(D, BD)
    max_n = _max_axis_chunks(grid_dim0)
    kernel_kwargs = dict(
        x=x,
        initial_state=initial_state,
