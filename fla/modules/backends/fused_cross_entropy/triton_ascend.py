...
def _block_size(n_cols: int, n_rows: int, is_backward: bool = False) -> tuple[int, int]:
...
def fused_cross_entropy_forward(
    ...
    BLOCK_SIZE, num_warps = _block_size(n_cols, n_rows)
...
def fused_cross_entropy_backward(
    ...
    n_rows, n_cols = logits.shape
    BLOCK_SIZE, num_warps = _block_size(n_cols, n_rows, is_backward=True)
    has_softcapping = logit_softcapping is not None
    softcap_val = float(logit_softcapping) if has_softcapping else 0.0
