...
| `FLA_CONFIG_DIR`     | -       | Any path                 | Override the default config directory for Triton kernel configs. When set, loads configs from `$FLA_CONFIG_DIR/{triton_version}/{GPU}/` instead of `fla/configs/{triton_version}/{GPU}/`. |
...
| `FLA_GPU_NAME`       | -       | Any string               | Override the detected GPU name for config directory naming. When set, configs will be stored in `configs/{triton_version}/{FLA_GPU_NAME}/` instead of auto-detecting from hardware (CUDA/ROCm). Useful for custom or unsupported devices. |
