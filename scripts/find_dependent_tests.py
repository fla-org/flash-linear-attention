...
def find_backend_op_files(changed_files: list[str], project_root: Path) -> list[str]:
    """Map backend changes to files importing their directory-owned dispatcher."""
    shared = any(file.startswith(('fla/backends/', 'fla/ops/backends/')) for file in changed_files)
    owners = set()
    ...
    for path in (project_root / 'fla').rglob('*.py'):
        if 'backends' in path.relative_to(project_root).parts:
            continue
        imports = get_imports_from_tree(parse_file(path))
        if any(symbol == 'dispatch' and (shared or module in owners) for module, symbol in imports):
            op_files.append(str(path.relative_to(project_root)))
    return op_files
