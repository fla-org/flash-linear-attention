...
- **No block pointers in Triton code**: write block accesses as explicit offset vectors with masked `tl.load` / `tl.store`; `tl.make_block_ptr` / `tl.advance` no longer compile (see CONTRIBUTING "Triton Kernels").
