...
- **RFC first for numerics changes**: anything that changes the numerical results of an existing, validated kernel — a different accumulation order or tiling, precision promotion/demotion, a relaxed tolerance, a swapped algorithm — needs an RFC issue before a PR, even when it stays within test tolerance. FLA's kernels are widely validated and downstream users rely on their exact numerics, so "faster and still within tolerance" is a design decision to be agreed on, not a fix to be shipped. This is stricter than the design-alignment rule above: for numerics, a draft PR is not a substitute for the RFC.
