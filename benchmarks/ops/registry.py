...
def _ce_logits_shape(B, T, H, D, **kw):
    return (B * T, D)


def _ce_input_shape(B, T, H, D, **kw):
    return (B * T, H)


def _ce_weight_shape(B, T, H, D, **kw):
    return (D, H)


def _ce_target_shape(B, T, H, D, **kw):
    return (B * T,)


def _ce_post_init(inputs, B, T, H, D, **kw):
...
_ce_shapes = {
    ...
    for N, V in [
        (1, 32000), (128, 32000), (1024, 4095), (1024, 4096), (1024, 4097),
        (1024, 65535), (1024, 65536), (1024, 65537), (2048, 100000), (8192, 128256),
    ]
}
_linear_ce_shapes = {
    f'N{N}_H{H}_V{V}': {'B': 1, 'T': N, 'H': H, 'D': V}
    for N, H, V in [
        (1, 512, 32000), (63, 512, 32000), (128, 256, 32000), (512, 2048, 32000),
        (1024, 1024, 32000), (1024, 2048, 65535), (1024, 2048, 65536), (1024, 2048, 65537),
        (2048, 2048, 100000), (4096, 4096, 128256), (8192, 2048, 32000), (16384, 2048, 32000),
    ]
}

register_op(OpConfig(
    name='cross_entropy_loss',
    import_path='fla.modules.fused_cross_entropy',
    inputs={
        'logits': TensorSpec(_ce_logits_shape),
        'target': TensorSpec(_ce_target_shape, requires_grad=False, dtype='long'),
    },
    post_init=_ce_post_init,
    default_shapes=_ce_shapes,
    test_file='tests/modules/test_cross_entropy.py',
    category='cross_entropy',
))

register_op(OpConfig(
    name='fused_linear_cross_entropy_loss',
    import_path='fla.modules.fused_linear_cross_entropy',
    inputs={
        'x': TensorSpec(_ce_input_shape),
        'target': TensorSpec(_ce_target_shape, requires_grad=False, dtype='long'),
        'weight': TensorSpec(_ce_weight_shape),
    },
    output_is_tuple=False,
    post_init=_ce_post_init,
    default_shapes=_linear_ce_shapes,
    test_file='tests/modules/test_cross_entropy.py',
    category='cross_entropy',
))
