...
import fla
from fla.models.rwkv7 import RWKV7ForCausalLM, RWKV7Config
import torch
import os
import json
import re
...
def convert(
    ...
    config.hidden_ratio = blinkdl_weights['blocks.0.ffn.key.weight'].shape[0] / blinkdl_weights['blocks.0.ffn.key.weight'].shape[1] # 4.0
    ...
    unused_weights = ['model.layers.0.attn.v_lora.lora.0.weight', 'model.layers.0.attn.v_lora.lora.2.weight', 'model.layers.0.attn.v_lora.lora.2.bias']
    ...
    for blink_name in blinkdl_weights:
        ...
        if re.match('.*[wva]0', blink_name): weight.squeeze_()
